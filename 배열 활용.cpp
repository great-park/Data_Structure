...
int main()
{
	while (true) {
		...
		if (x == 1) {
			...
			//printf("추가할 전화번호: "); scanf("%s", phonenumber);
			//printf("추가할 주소: "); scanf("%s", address);
			//printf("추가할 생일: "); scanf("%s", birthday);

			//strcpy(book[size].name, name);
			//strcpy(book[size].address, address);
			//strcpy(book[size].birth, birthday);
			//strcpy(book[size].num, phonenumber);
